...
class Encoder(nn.Module):
    ...
    def forward(self, input, label, protocol):
        x = [None]*protocol['num_node']
        output = input.new_zeros(input.size(0),input.size(1),code_size,4,4)
        for i in range(protocol['num_node']):
...
class Decoder(nn.Module):
    ...
    def forward(self, input, label, protocol):
        x = [None]*protocol['num_node']
        output = input.new_zeros(input.size(0),input.size(1),1,32,32) if (protocol['mode'] == 'L') else input.new_zeros(input.size(0),input.size(1),3,32,32)
        for i in range(protocol['num_node']):
            if(input[label==i].size(0)==0):
                continue
            x[i] = input[label==i]
...
class Joint_dist_cor_base(nn.Module):
    def __init__(self,classes_size):
        super(Joint_dist_cor_base, self).__init__()
        self.classes_size = classes_size
        self.codec = Codec()
    ...
    def forward(self, input, protocol):
        ...
        for i in range(protocol['num_iter']):
            ...
            encoded = self.codec.encoder(compression_residual,input['label'],protocol)
