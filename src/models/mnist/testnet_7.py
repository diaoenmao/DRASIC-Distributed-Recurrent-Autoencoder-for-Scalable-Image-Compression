...
class Classifier(nn.Module):
    ...
    def make_classifier_info(self):
        ...
        [{'input_size':code_size,'output_size':10,'num_layer':1,'cell':cell_name,'mode':'fc','normalization':'none','activation':'none','raw':True}],       
        ]
        classifier_info = [
        {'cell': 'ResLSTMCell','num_layer':2,'in': classifier_in_info[0],'hidden':classifier_hidden_info[0],'shortcut':classifier_shortcut_info[0],'activation':'tanh'},
        ]
...
class testnet_7(nn.Module):
    def __init__(self,classes_size):
        super(testnet_7, self).__init__()
        self.classes_size = classes_size
        self.codec = Codec()
