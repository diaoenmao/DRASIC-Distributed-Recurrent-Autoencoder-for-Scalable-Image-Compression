...
class Encoder(nn.Module):
    ...
    def forward(self, input, protocol):
        ...
        for i in range(len(x)):
            ...
            x[i] = self.encoder[encoder_idx][0](x[i], protocol)
            for j in range(1,protocol['depth']+1):
                x[i] = apply_along_dim(x[i], protocol['jump_rate'], fn=pixel_unshuffle, dim=1)
                x[i] = self.encoder[encoder_idx][j](x[i], protocol)
            x[i] = self.encoder[encoder_idx][-1](x[i], protocol)
        x = torch.cat(x,dim=0)
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self, input, protocol):
        ...
        decoder_idx = 7
        for i in range(len(x)):
            x[i] = self.decoder[decoder_idx][0](x[i], protocol)
            for j in range(1,protocol['depth']+1):
                x[i] = self.decoder[decoder_idx][j](x[i], protocol)
                x[i] = apply_along_dim(x[i], protocol['jump_rate'], fn=F.pixel_shuffle, dim=1)
            x[i] = self.decoder[decoder_idx][-1](x[i], protocol)
...
class Joint_dist_base(nn.Module):
    def __init__(self,classes_size):
        super(Joint_dist_base, self).__init__()
        self.classes_size = classes_size
        self.codec = Codec()
