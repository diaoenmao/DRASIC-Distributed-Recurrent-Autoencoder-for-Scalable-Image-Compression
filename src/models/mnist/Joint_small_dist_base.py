...
num_node = config.PARAM['train_num_node']
...
class Encoder(nn.Module):
    ...
    def make_encoder_info(self):
        ...
        [{'model':cell_name,'input_size':128,'output_size':32,'num_layers':1,'kernel_size':3,'hidden_kernel_size':3,'stride':1,'hidden_stride':1,'padding':1,'hidden_padding':1,'dilation':1,'bias':False}],
        [{'model':cell_name,'input_size':128,'output_size':32,'num_layers':1,'kernel_size':3,'hidden_kernel_size':3,'stride':1,'hidden_stride':1,'padding':1,'hidden_padding':1,'dilation':1,'bias':False}],
        [{'model':cell_name,'input_size':128,'output_size':32,'num_layers':1,'kernel_size':3,'hidden_kernel_size':3,'stride':1,'hidden_stride':1,'padding':1,'hidden_padding':1,'dilation':1,'bias':False}],
        [{'model':'Conv2d','input_size':32,'output_size':code_size,'num_layers':1,'kernel_size':1,'stride':1,'padding':0,'dilation':1,'bias':False}]
        ]
        return encoder_info
    ...
    def make_encoder(self):
        ...
        for i in range(num_node):
            ...
            for j in range(len(self.encoder_info)):
                node_encoder.append(EncoderCell(self.encoder_info[j]))
            encoder.append(node_encoder)
        return encoder

    def free_hidden(self):
        for i in range(len(self.encoder)):
            for j in range(len(self.encoder[i])):
                self.encoder[i][j].free_hidden()
        return
        
    def forward(self, input, protocol):
        x = list(input.chunk(protocol['num_node'],dim=0))
        for i in range(len(x)):
            x[i] = apply_along_dim(x[i],fn=L_to_RGB,dim=1) if (protocol['mode'] == 'L') else x[i]
            x[i] = self.encoder[i][0](x[i], protocol)
            for j in range(1,protocol['depth']+1):
                x[i] = apply_along_dim(x[i], protocol['jump_rate'], fn=pixel_unshuffle, dim=1)
                x[i] = self.encoder[i][j](x[i], protocol)
            x[i] = self.encoder[i][-1](x[i], protocol)
        x = torch.cat(x,dim=0)
        return x
...
class Decoder(nn.Module):
    ...
    def make_decoder(self):
        ...
        for i in range(num_node):
            node_decoder = nn.ModuleList([])
            for j in range(len(self.decoder_info)):
                node_decoder.append(DecoderCell(self.decoder_info[j]))
            decoder.append(node_decoder)
        return decoder

    def free_hidden(self):
        for i in range(len(self.decoder)):
            for j in range(len(self.decoder[i])):
                self.decoder[i][j].free_hidden()
        return
        
    def forward(self, input, protocol):
        x = list(input.chunk(protocol['num_node'],dim=0))
        for i in range(len(x)):
            x[i] = self.decoder[i][0](x[i], protocol)
            for j in range(1,protocol['depth']+1):
                x[i] = self.decoder[i][j](x[i], protocol)
                x[i] = apply_along_dim(x[i], protocol['jump_rate'], fn=F.pixel_shuffle, dim=1)
            x[i] = self.decoder[i][-1](x[i], protocol)
            x[i] = apply_along_dim(x[i],fn=RGB_to_L,dim=1) if (protocol['mode'] == 'L') else x[i]
        x = torch.cat(x,dim=0)
        return x
...
class Classifier(nn.Module):
    ...
    def make_classifier_info(self):
        classifier_info = [
                        [{'model':cell_name,'input_size':code_size,'output_size':512,'num_layers':1,'kernel_size':3,'hidden_kernel_size':3,'stride':1,'hidden_stride':3,'padding':1,'hidden_padding':3,'dilation':1,'bias':False}],
...
class Joint_small_dist_base(nn.Module):
    def __init__(self,classes_size):
        super(Joint_small_dist_base, self).__init__()
        self.classes_size = classes_size
        self.codec = Codec()
