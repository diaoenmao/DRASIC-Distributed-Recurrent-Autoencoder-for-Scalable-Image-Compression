...
class Attention(nn.Module):
    def __init__(self, attention_info):
        super(Attention, self).__init__()
        self.attention_info = attention_info
        self.attention = self.make_attention()
            
    def make_attention(self):
        attention = nn.ModuleList([])
        for i in range(len(self.attention_info)):
            attention.append(Cell(self.attention_info[i]))
        return attention
    
    def free_hidden(self):
        for i in range(len(self.attention_info)):
            self.attention[i].hidden = None
        return
        
    def attend(self, x_output, x_attention, dim):
        x = x_output.new_zeros(x_output.size())
        for i in range(x_output.size(dim)):
            x[:,i] = (x_output[:,:(i+1)] * F.softmax(x_attention[:,:(i+1)],dim=dim)).sum(dim=dim)
        return x
    ...
    def forward(self, input, protocol):
        ...
        for i in range(len(self.attention)):
            if(i==len(self.attention)-1):
                protocol['activation'] = 'none'                
            x = self.attention[i](x,protocol)
...
class Classifier(nn.Module):
    def __init__(self, classes_size):
        ...
        self.attention_info = self.make_attention_info()
    ...
    def make_attention_info(self):
        attention_info = [
        {'model':'Conv2d','input_size':512,'output_size':512,'num_layers':1,'kernel_size':1,'hidden_kernel_size':1,'stride':1,'padding':0,'dilation':1,'bias':False},
        {'model':'Conv2d','input_size':512,'output_size':1,'num_layers':1,'kernel_size':1,'hidden_kernel_size':1,'stride':1,'padding':0,'dilation':1,'bias':False}
        ]
    ...
    def make_attention(self):
        attention = Attention(self.attention_info)
        return attention
        
    def free_hidden(self):
        for i in range(len(self.classifier)):
            self.classifier[i].free_hidden()
        self.attention.free_hidden()
        return
    ...
    def forward(self, input, protocol):
        ...
        x_attention = self.attention(x,protocol)
        x = self.attention.attend(x,x_attention,dim=1)    
...
class Joint_attention(nn.Module):
    ...
    def forward(self, input, protocol):
        ...
        output['compression']['img'] = output['compression']['img'][:,-1]
        self.codec.free_hidden()
        
        classification_loss = 0
        if(protocol['tuning_param']['classification'] > 0):
            output['classification'] = self.classifier(output['compression']['code'],protocol)
            classification_loss = self.classifier.classification_loss_fn(input,output,protocol)     
            output['classification'] = output['classification'][:,-1]
