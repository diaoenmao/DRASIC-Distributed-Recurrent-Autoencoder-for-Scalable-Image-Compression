...
def runExperiment(seed):
    ...
    _,test_loader = split_dataset(train_dataset,test_dataset,valid_data_size,batch_size=batch_size,radomGen=randomGen,shuffle=False)
...
def test(validation_loader,model,epoch,protocol,iter,model_TAG):
    ...
    with torch.no_grad():
        ...
        for i, input in enumerate(validation_loader):
            input = collate(input)
            input['img'] = input['img'][input['label']<protocol['num_node']]
            input['label'] = input['label'][input['label']<protocol['num_node']] 
